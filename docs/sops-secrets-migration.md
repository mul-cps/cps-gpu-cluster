...
- [x] JupyterHub OAuth (Authentik) client_secret (Authentik "CPS-CIT-JupyterHub" provider, pk 33, client_id `vUhzKqEF0UxPtZNM8aRbA1ncaehhIAIA2x9r83FI`) — rotated 2026-07-23 in Authentik (client_id unchanged, only the secret rotated); superseded the "reused live value (not rotated)" note above (row 31 of the inventory table) — this client_secret was one of the "Secondary secret exposures" found in `docs/rancher-authentik-sso-plan.md` (leaked in git history before the `values.yaml` fix removed it from plaintext). `jupyter/jupyterhub/oauth-sopssecret.yaml` re-encrypted with the rotated value.
